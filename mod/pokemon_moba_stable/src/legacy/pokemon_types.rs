...
use crate::stable_legacy_api::{AttackType, GameCtx};
...
pub fn deal_pokemon_damage(
    ...
    attacker_types: TypeSet,
    defender_types: TypeSet,
) -> crate::legacy::pokemon_status::PokemonDamageResult {
    let (target, defender_types) =
        if let Some(audino_id) = crate::legacy::pokemon_status::audino_protect_redirect(ctx, target) {
            (
                audino_id,
                crate::legacy::neutral_objectives::defender_types_for_target(ctx, audino_id),
            )
        } else {
            (target, defender_types)
        };

    if crate::legacy::pokemon_status::telepathy_blocks_ally_harm(ctx, attacker, target) {
        return crate::legacy::pokemon_status::PokemonDamageResult::default();
    }

    if crate::legacy::pokemon_status::consume_detect_guard(ctx, target) {
        return crate::legacy::pokemon_status::PokemonDamageResult::default();
    }
    if crate::legacy::pokemon_status::try_consume_zeraora_zing_zap(ctx, target, attacker, attack_type) {
        return crate::legacy::pokemon_status::PokemonDamageResult::default();
    }
    if crate::legacy::pokemon_status::is_bouffalant_unstoppable(ctx, target) {
        return crate::legacy::pokemon_status::PokemonDamageResult::default();
    }

    if let Some(retaliation_damage) =
        crate::legacy::pokemon_status::try_consume_reversal(ctx, target, attacker)
    {
        let retaliation_attacker_types =
            crate::legacy::neutral_objectives::defender_types_for_target(ctx, target);
        let retaliation_defender_types =
            crate::legacy::neutral_objectives::defender_types_for_target(ctx, attacker);
        deal_pokemon_damage(
            ctx,
            target,
            attacker,
    ...
    if (ad_damage > 0 || ap_damage > 0)
        && crate::legacy::pokemon_status::try_wonder_guard_damage(
            ctx,
            attacker,
            target,
            attack_type,
    ...
    let (modified_ad_damage, modified_ap_damage) =
        if crate::legacy::pokemon_status::energy_predator_forces_super_effective(ctx, attacker, target) {
            let (stab_num, stab_den) = stab_modifier_ratio(move_type, attacker_types);
    ...
    let bypass_resistance = crate::legacy::pokemon_status::is_blood_moon_active(ctx, attacker)
        || crate::legacy::pokemon_status::has_scrappy(ctx, attacker)
        || crate::legacy::pokemon_status::thievul_stakeout_bypasses_resistance(ctx, attacker, target);
    ...
    let energy_predator_bonus =
        crate::legacy::pokemon_status::energy_predator_damage_bonus_percent(ctx, attacker, target);
    ...
    let aqua_step_bonus = if matches!(move_type, PokemonType::Water) {
        crate::legacy::pokemon_status::quaquaval_water_damage_bonus_percent(ctx, attacker)
    } else {
        0
    };
    ...
    let reduce_percent =
        crate::legacy::pokemon_status::gallade_prediction_reduce_percent(ctx, target, attacker);
    ...
    let reduce_percent = crate::legacy::pokemon_status::wide_guard_reduce_percent(ctx, target, attacker);
    let (ad_damage, ap_damage) = if reduce_percent > 0 {
        (
            ad_damage.saturating_mul(100usize.saturating_sub(reduce_percent)) / 100,
            ap_damage.saturating_mul(100usize.saturating_sub(reduce_percent)) / 100,
    ...
    let reduce_percent = crate::legacy::pokemon_status::afro_reduce_percent(ctx, target, attacker);
    let (ad_damage, ap_damage) = if reduce_percent > 0 {
        (
            ad_damage.saturating_mul(100usize.saturating_sub(reduce_percent)) / 100,
            ap_damage.saturating_mul(100usize.saturating_sub(reduce_percent)) / 100,
    ...
    let reduce_percent = crate::legacy::pokemon_status::bouffalant_retaliate_reduce_percent(ctx, target);
    let (ad_damage, ap_damage) = if reduce_percent > 0 {
        (
            ad_damage.saturating_mul(100usize.saturating_sub(reduce_percent)) / 100,
            ap_damage.saturating_mul(100usize.saturating_sub(reduce_percent)) / 100,
    ...
    let (ad_damage, ap_damage) = if matches!(move_type, PokemonType::Fighting)
        && crate::legacy::pokemon_status::has_boxer(ctx, target)
    {
        (
    ...
    let reduce_percent = crate::legacy::pokemon_status::sirfetchd_leek_damage_reduce_percent(ctx, target);
    let (ad_damage, ap_damage) = if reduce_percent > 0 {
        (
            ad_damage.saturating_mul(100usize.saturating_sub(reduce_percent)) / 100,
            ap_damage.saturating_mul(100usize.saturating_sub(reduce_percent)) / 100,
    ...
    let reduce_percent =
        crate::legacy::pokemon_status::try_trigger_hawlucha_counter(ctx, target, attacker, attack_type);
    let (ad_damage, ap_damage) = if reduce_percent > 0 {
        (
            ad_damage.saturating_mul(100usize.saturating_sub(reduce_percent)) / 100,
            ap_damage.saturating_mul(100usize.saturating_sub(reduce_percent)) / 100,
    ...
    let ap_damage = if crate::legacy::pokemon_status::has_ice_scales(ctx, target) {
        ap_damage / 2
    } else {
        ap_damage
    };
    let ad_damage =
        ad_damage.saturating_add(crate::legacy::pokemon_status::bouffalant_head_charge_bonus_damage(
            ctx,
            attacker,
            target,
            attack_type,
    ...
    let (ad_damage, ap_damage) =
        crate::legacy::pokemon_status::adjust_endure_damage(ctx, target, ad_damage, ap_damage);
    let (ad_damage, ap_damage) =
        crate::legacy::pokemon_status::adjust_sturdy_damage(ctx, target, attack_type, ad_damage, ap_damage);
    let pain_amp_bonus = crate::legacy::pokemon_status::xatu_pain_amplifier_bonus_percent(ctx, target);
    ...
    let (ad_damage, ap_damage) =
        crate::legacy::pokemon_status::absorb_light_metal_damage(ctx, target, ad_damage, ap_damage);
    if ad_damage > 0 || ap_damage > 0 {
        crate::legacy::pokemon_status::note_light_metal_dealt_damage(ctx, attacker);
        crate::legacy::pokemon_status::note_direct_pokemon_damage(
            ctx,
            attacker,
            target,
        ...
        crate::legacy::pokemon_status::note_starmie_damage_hit(ctx, attacker, target);
        crate::legacy::pokemon_status::trigger_justified_if_dark_damage(
            ctx,
            target,
            move_type,
            ad_damage.saturating_add(ap_damage),
        );
        crate::legacy::pokemon_status::trigger_steam_engine_if_relevant(ctx, target, move_type);
        crate::legacy::pokemon_status::trigger_flash_fire_if_relevant(
            ctx,
            target,
            move_type,
            ad_damage.saturating_add(ap_damage),
    ...
    let stat_probe_enabled = crate::legacy::crash_probe::stat_probe_damage_enabled();
    let stat_probe_before = if stat_probe_enabled {
        let attacker_player = crate::legacy::pokemon_status::player_for_entity(attacker);
        let target_player = crate::legacy::pokemon_status::player_for_entity(target);
        Some((
            attacker_player,
            target_player,
    ...
    let damage_result = crate::legacy::pokemon_status::deal_tracked_damage(
        ctx,
        attacker,
        target,
        ad_damage,
    ...
    if damage_result.applied_damage > 0 && ad_damage > 0 {
        crate::legacy::pokemon_status::trigger_garganacl_salt_cure_from_physical_damage(
...
fn log_pokemon_damage_probe(
    ...
    let attacker_player = crate::legacy::pokemon_status::player_for_entity(attacker).unwrap_or(attacker);
    let target_player = crate::legacy::pokemon_status::player_for_entity(target).unwrap_or(target);
    let attacker_champion =
        crate::legacy::pokemon_status::champion_id_for_entity(attacker).unwrap_or("unknown");
    let target_champion =
        crate::legacy::pokemon_status::champion_id_for_entity(target).unwrap_or("unknown");
    crate::legacy::crash_probe::log_damage_probe(&format!(
        ...
        attacker,
        attacker_player,
        crate::legacy::crash_probe::sanitize_log_field(attacker_champion),
        target,
        target_player,
