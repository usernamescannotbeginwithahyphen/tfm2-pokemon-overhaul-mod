...
use crate::stable_legacy_api::{
    AttackType, BuffState, BuffType, CCState, EntityPos, GameCtx, PlayerState, Position,
};

use crate::legacy::pokemon_content::{
    champion_base_stat_for_level, ActionSlot, PokemonChampion, PokemonMove, PokemonMoveEffect,
};
use crate::legacy::pokemon_types::{PokemonType, TypeSet};
...
const ATTACH_TETHER_DEADZONE: u64 = 500;
const ATTACH_TETHER_SPEED: u64 = 3_600_000;
const ATTACH_TETHER_TICKS: u64 = 4;
...
pub fn try_wonder_guard_damage(
    ...
    if state.knocked_until > tick {
        ...
        if state.chits == 0 {
            ...
            drop(states);
            crate::legacy::pokemon_status::deal_tracked_damage(
                ctx,
                attacker_id,
                target_id,
                1_000_000,
    ...
    let super_effective = crate::legacy::pokemon_types::type_modifier_ratio(move_type, defender_types).0
        > crate::legacy::pokemon_types::type_modifier_ratio(move_type, defender_types).1;
...
pub fn try_wonder_guard_dot_tick(
    ...
    let defender_types = crate::legacy::neutral_objectives::defender_types_for_target(ctx, target_id);
...
pub fn heal_with_antiheal(
    ...
    if adjusted > 0 {
        ...
        if crate::legacy::crash_probe::damage_probe_enabled() {
            crate::legacy::crash_probe::log_damage_probe(&format!(
                ...
                caster_id,
                caster_player,
                crate::legacy::crash_probe::sanitize_log_field(caster_champion),
                target_id,
                target_player,
                crate::legacy::crash_probe::sanitize_log_field(target_champion),
                amount,
                adjusted,
                heal_amount,
                percent,
        ...
        crate::legacy::crash_probe::log_stat_probe(&format!(
            "event=pokemon_heal tick={} caster={} caster_player={} caster_champion=\"{}\" target={} target_player={} target_champion=\"{}\" requested={} adjusted={} native_heal={} anti_heal_percent={} before_hp={} after_hp={} hp_gained={}",
            ctx.tick(),
            caster_id,
            caster_player,
...
fn log_dot_absorb_heal(
    ...
    if crate::legacy::crash_probe::damage_probe_enabled() {
        ...
        crate::legacy::crash_probe::log_damage_probe(&format!(
            ...
            crate::legacy::crash_probe::sanitize_log_field(dot_type),
            ctx.tick(),
            original_caster_id,
            original_caster_player,
            crate::legacy::crash_probe::sanitize_log_field(original_caster_champion),
            original_target_id,
            original_target_player,
            crate::legacy::crash_probe::sanitize_log_field(original_target_champion),
            healed_entity_id,
            healed_player,
            crate::legacy::crash_probe::sanitize_log_field(healed_champion),
            amount,
        ));
    }
}
...
pub fn apply_ampharos_flash(
    ...
    for target_id in targets {
        let defender_types = crate::legacy::neutral_objectives::defender_types_for_target(ctx, target_id);
        crate::legacy::pokemon_types::deal_pokemon_damage(
            ctx,
            caster_id,
            target_id,
            0,
...
fn update_yanmega_giga_drains(ctx: &mut GameCtx, tick: usize) {
    ...
    for mut state in ticks_to_apply {
        ...
        let defender_types =
            crate::legacy::neutral_objectives::defender_types_for_target(ctx, state.target_id);
        let result = crate::legacy::pokemon_types::deal_pokemon_damage(
            ctx,
            state.caster_id,
            state.target_id,
            0,
...
fn update_swanna_cyclones(ctx: &mut GameCtx, tick: usize) {
    ...
    for (caster_id, damage, slow_percent, slow_ticks, attacker_types, targets) in ticks {
        for target_id in targets {
            let defender_types =
                crate::legacy::neutral_objectives::defender_types_for_target(ctx, target_id);
            crate::legacy::pokemon_types::deal_pokemon_damage(
                ctx,
                caster_id,
                target_id,
...
fn update_marowak_bone_windmills(ctx: &mut GameCtx, tick: usize) {
        ...
        ticks
    {
        for (target_id, target_pos) in targets {
            ...
            let defender_types =
                crate::legacy::neutral_objectives::defender_types_for_target(ctx, actual_target_id);
            crate::legacy::pokemon_types::deal_pokemon_damage(
                ctx,
                caster_id,
                actual_target_id,
...
fn missingno_chain_damage(
    ...
    for jump in 0..=chain_jumps {
        ...
        if target_team == caster_team {
            ...
            let defender_types =
                crate::legacy::neutral_objectives::defender_types_for_target(ctx, current_id);
            crate::legacy::pokemon_types::deal_pokemon_damage(
                ctx,
                caster_id,
                current_id,
...
fn symbiosis_item_share_buff(
    current: crate::stable_legacy_api::EntityStat,
    base: crate::stable_legacy_api::EntityStat,
    ticks: usize,
) -> BuffState {
    BuffState {
        duration: BuffType::Time { tick: ticks },
...
pub fn deal_tracked_damage(
    ...
    if crate::legacy::crash_probe::stat_probe_enabled()
...
fn log_stat_probe_damage_transition(
    ...
    crate::legacy::crash_probe::log_stat_probe_event(&format!(
        ...
        target_id,
        damage_credit,
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_damage_snapshot_value(before)),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_damage_snapshot_value(after)),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_info_value(attacker_info)),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_info_value(target_info)),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_death_delta_value(death_before, death_after)),
        kill_log_before,
        kill_log_after,
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_native_kda_value(
            ctx,
            attacker_info.and_then(|info| info.player_id)
        )),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_custom_kda_value(
            ctx,
            attacker_info.and_then(|info| info.player_id)
        )),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_native_kda_value(
...
fn record_pokemon_kill_credit(
    ...
    let assist_ids = crate::legacy::crash_probe::catch_unwind_probe(
    ...
    for assist_player_id in &assist_ids {
        crate::legacy::crash_probe::catch_unwind_probe(
    ...
    crate::legacy::crash_probe::log_kda_probe(&format!(
        ...
        crate::legacy::crash_probe::sanitize_log_field(origin),
        tick,
        combat_ctx_id(ctx),
        killer_id,
        killer_player_id,
        crate::legacy::crash_probe::sanitize_log_field(killer_champion),
        killed_id,
        killed_player_id,
        killed_generation,
        crate::legacy::crash_probe::sanitize_log_field(killed_champion),
        assist_ids
            .iter()
        ...
        crate::legacy::crash_probe::sanitize_log_field(&kda_probe_recent_credits(
            ctx,
            tick,
            killed_id,
            Some(killed_player_id),
...
fn log_kda_credit_skip(
    ...
    crate::legacy::crash_probe::log_stat_probe_event(&format!(
        ...
        crate::legacy::crash_probe::sanitize_log_field(origin),
        crate::legacy::crash_probe::sanitize_log_field(reason),
        ctx.tick(),
        combat_ctx_id(ctx),
        killer_id,
        killed_id,
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_info_value(killer_info)),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_info_value(killed_info)),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_native_kda_value(
            ctx,
...
pub fn record_native_pokemon_kill_assists(
    ...
    let Some(killer_info) = combat_stat_player_info(ctx, killer_player_id) else {
        crate::legacy::crash_probe::log_stat_probe_event(&format!(
            ...
            killer_player_id,
            killed_entity_id,
            crate::legacy::crash_probe::sanitize_log_field(&stat_probe_recent_kill_log_value(ctx)),
        ));
        return;
    };
    ...
    crate::legacy::crash_probe::log_stat_probe_event(&format!(
        ...
        killer_player_id,
        killed_entity_id,
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_info_value(Some(killer_info))),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_info_value(direct_killed_info)),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_info_value(fallback_killed_info)),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_recent_kill_log_value(ctx)),
        crate::legacy::crash_probe::sanitize_log_field(&stat_probe_native_kda_value(
            ctx,
            killer_info.player_id
...
fn award_pokemon_custom_kill_gold(
    ...
    if !ctx.player_add_gold(killer_player_id, amount as i64) {
        crate::legacy::crash_probe::log_stat_probe_event(&format!(
            "event=custom_kill_gold_skip reason=stable_add_gold_failed tick={} ctx={} origin={} killer_player={} killed_player={} killed_life={} amount={}",
            ctx.tick(),
            ctx_id,
            crate::legacy::crash_probe::sanitize_log_field(origin),
            killer_player_id,
    ...
    let after_gold = ctx
        .get_player(killer_player_id)
        .map(|updated| updated.gold())
        .unwrap_or(before_gold.saturating_add(amount));
    crate::legacy::crash_probe::log_stat_probe_event(&format!(
        "event=custom_kill_gold_apply tick={} ctx={} origin={} killer_player={} killed_player={} killed_life={} amount={} gold_before={} gold_after={}",
        ctx.tick(),
        ctx_id,
        crate::legacy::crash_probe::sanitize_log_field(origin),
        killer_player_id,
...
fn add_pokemon_bonus_gold_for_player(
    ...
    let Some(player) = ctx.get_player(player_id) else {
        crate::legacy::crash_probe::log_stat_probe_event(&format!(
            ...
            amount,
            crate::legacy::crash_probe::sanitize_log_field(reason),
            crate::legacy::crash_probe::sanitize_log_field(detail)
        ));
        return false;
    };
    let before_gold = player.gold();
    if !ctx.player_add_gold(player_id, amount as i64) {
        crate::legacy::crash_probe::log_stat_probe_event(&format!(
            "event=bonus_gold_skip reason=stable_add_gold_failed tick={} ctx={} player={} amount={} source={} detail=\"{}\"",
            ctx.tick(),
            ctx_id,
            player_id,
            amount,
    ...
    let after_gold = ctx
        ...
        .unwrap_or(before_gold.saturating_add(amount));
    crate::legacy::crash_probe::log_stat_probe_event(&format!(
        "event=bonus_gold_apply tick={} ctx={} player={} amount={} source={} detail=\"{}\" gold_before={} gold_after={}",
        ctx.tick(),
        ctx_id,
        player_id,
        amount,
...
fn spend_pokemon_live_gold_for_player(
    ...
    if !ctx.player_add_gold(player_id, -(spent as i64)) {
        crate::legacy::crash_probe::log_stat_probe_event(&format!(
            "event=live_gold_spend_skip reason=stable_add_gold_failed tick={} ctx={} player={} requested={} source={} detail=\"{}\"",
            ctx.tick(),
            ctx_id,
            player_id,
            amount,
    ...
    let after_gold = ctx
        .get_player(player_id)
        .map(|updated| updated.gold())
        .unwrap_or(before_gold.saturating_sub(spent));
    crate::legacy::crash_probe::log_stat_probe_event(&format!(
...
fn log_pokemon_base_stat_sync_event(message: &str) {
    ...
    *count += 1;
    crate::legacy::crash_probe::log_stat_probe_event(message);
}
...
pub fn try_consume_zeraora_zing_zap(
    ...
    let defender_types = crate::legacy::neutral_objectives::defender_types_for_target(ctx, attacker_id);
    crate::legacy::pokemon_types::deal_pokemon_damage(
        ctx,
        entity_id,
        attacker_id,
...
fn trigger_zeraora_merciless(ctx: &mut GameCtx, caster_id: usize, target_id: usize) {
    ...
    for hit_id in hits.iter().copied() {
        let defender_types = crate::legacy::neutral_objectives::defender_types_for_target(ctx, hit_id);
        crate::legacy::pokemon_types::deal_pokemon_damage(
            ctx,
            caster_id,
            hit_id,
...
fn explode_sigilyph_glyph(ctx: &mut GameCtx, caster_id: usize, target_id: usize) {
    ...
    if splash_damage > 0 {
        ...
        for splash_target_id in splash_targets {
            let defender_types =
                crate::legacy::neutral_objectives::defender_types_for_target(ctx, splash_target_id);
            crate::legacy::pokemon_types::deal_pokemon_damage(
                ctx,
                caster_id,
                splash_target_id,
    ...
    for state in resolving_retaliates {
        ...
        for (attacker_id, stored_damage) in state.attackers {
            ...
            let damage = stored_damage
                ...
                .max(1);
            let defender_types =
                crate::legacy::neutral_objectives::defender_types_for_target(ctx, attacker_id);
            crate::legacy::pokemon_types::deal_pokemon_damage(
                ctx,
                state.entity_id,
                attacker_id,
...
fn update_grapploct_submissions(ctx: &mut GameCtx, tick: usize) {
    ...
    for state in ticks {
        ...
        {
            let probe_enabled = crate::legacy::crash_probe::damage_probe_enabled();
...
fn distance_sq(a: crate::stable_legacy_api::EntityPos, b: crate::stable_legacy_api::EntityPos) -> u64 {
...
fn distance_to_segment_sq(
    point: crate::stable_legacy_api::EntityPos,
    start: crate::stable_legacy_api::EntityPos,
    end: crate::stable_legacy_api::EntityPos,
) -> u64 {
